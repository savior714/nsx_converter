# 🗒️ Synology Note Station → Markdown 변환기

Synology Note Station의 `.nsx` 백업 파일을 Markdown 또는 HTML 파일로 변환하는 GUI 프로그램입니다.

## ✨ 주요 기능

- 📦 `.nsx` 파일 자동 압축 해제
- 📝 JSON 형식의 노트 파일 파싱
- 🔄 HTML → Markdown 자동 변환 (Pandoc 사용)
- 🖼️ **이미지 자동 추출 및 경로 수정 (PNG 포함)**
- 🌐 웹 브라우저 기반 GUI 인터페이스
- 📊 실시간 변환 진행 상황 표시
- ⚡ 중복 파일명 자동 처리
...
HTML 추출 및 경로 수정 → Pandoc 변환 → .md 파일 생성
```
...
- HTML/Markdown 파일 내의 이미지 경로가 자동으로 상대 경로로 수정됩니다
...
# 또는 Winget 사용 (Windows 10/11)
winget install pandoc

# 또는 공식 사이트에서 설치 프로그램 다운로드
...
- 중복된 파일 이름은 자동으로 번호가 추가됩니다 (예: `노트_1.md`, `노트_2.md`)
- 이미지는 `webman` 폴더에 저장되므로 HTML/Markdown 파일과 함께 유지해야 합니다
- 변환된 파일을 이동할 때는 `webman` 폴더도 함께 이동해야 이미지가 표시됩니다

## 🐛 문제 해결

### "Pandoc이 설치되어 있지 않습니다"
- Pandoc을 설치하거나 HTML 파일로 저장하도록 선택하세요
